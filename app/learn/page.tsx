...
export default function LearnPage() {
  ...
  const { user, updateUserProgress, updateUserXP } = useUser()
  ...
  const trackAction = async (actionType: string, xpGained: number) => {
    ...
    try {
      // First, track the action in the database
      ...
      // Then update the user's XP using our new method
      const result = await updateUserXP(xpGained, `${actionType.replace("_", " ")}`)
  ...
  const handleSectionComplete = async (sectionName: string, xpGained = 10) => {
    if (!completedSections.includes(sectionName) && videoId && user) {
      ...
      await trackAction(`complete_${sectionName}`, xpGained)
