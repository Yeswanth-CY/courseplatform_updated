...
import { useXPUpdates } from "@/hooks/use-xp-updates"
...
export default function ProfilePage() {
  ...
  const { lastUpdate, isConnected, refreshUserData } = useXPUpdates()
  ...
  // Use the lastUpdate from our hook to trigger achievement refreshes
  useEffect(() => {
    if (lastUpdate && user) {
      fetchAchievements()
    }
  ...
  const fetchAchievements = async () => {
    ...
    try {
      // Use the cached achievement progress calculation
              ...
              <div className="ml-32">
                ...
                <div className="space-y-2">
                  <div className="flex justify-between text-sm">
                    ...
                    <span>
                      {currentLevelProgress.toLocaleString()} / {nextLevelXP.toLocaleString()} XP
                    </span>
                  </div>
                  ...
                  <p className="text-xs text-white/60">
                    {(nextLevelXP - currentLevelProgress).toLocaleString()} XP until level {currentLevel + 1}
                  </p>
                </div>
              </div>
            </CardContent>
