...
export class XPCalculator {
  ...
  // Cache for level calculations to improve performance
  private static levelCache: Record<
    number,
    {
      level: number
  ...
  static calculateLevel(totalXP: number): {
    ...
    // Check cache first
    if (this.levelCache[totalXP]) {
      return this.levelCache[totalXP]
    }
    ...
    // Cache the result
    const result = { level, currentLevelXP, nextLevelXP, progress }
    this.levelCache[totalXP] = result

    return result
  }

  // Clear cache when needed (e.g., when XP formula changes)
  static clearCache() {
    this.levelCache = {}
  }
