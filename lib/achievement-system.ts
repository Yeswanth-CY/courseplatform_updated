...
export class AchievementSystem {
  ...
  // Cache for achievement progress to improve performance
  private static achievementCache: Record<
    string,
    Array<Achievement & { current_progress: number; unlocked: boolean }>
  > = {}
  ...
  static getAchievementProgress(userStats: any): Array<Achievement & { current_progress: number; unlocked: boolean }> {
    // Generate a cache key based on relevant user stats
    const cacheKey = `${userStats.id}-${userStats.total_xp}-${userStats.videos_watched}-${userStats.current_streak}`

    // Check if we have a cached result
    if (this.achievementCache[cacheKey]) {
      return this.achievementCache[cacheKey]
    }

    // Calculate achievement progress
    const result = this.ACHIEVEMENTS.map((achievement) => ({
      ...achievement,
    ...
    // Cache the result
    this.achievementCache[cacheKey] = result
  ...
  // Method to update cache for a specific user
  static updateUserCache(userId: string) {
    // Remove all cache entries for this user
    Object.keys(this.achievementCache).forEach((key) => {
      if (key.startsWith(`${userId}-`)) {
        delete this.achievementCache[key]
      }
    })
  }
}
